	...
	for edge in graph.get_edges():
		...
		if (vertex1 != null and vertex2 != null) and (vertex1.subOf == null and vertex2.subOf == null) and (edge.type == TYPE_EDGE.PATH) :
			if edge.type == TYPE_EDGE.PATH:
				...
				file.store_line('"'+vertex_dot(vertex1)+'"' + " -> " + '"'+vertex_dot(vertex2)+'" [ style=dashed ]'+' [label = "'+str(edge.weight)+'"]')
...
#func _find_cycle(graph: Node):
#	var adj_list: Dictionary =  graph.get_adjacent_list()
